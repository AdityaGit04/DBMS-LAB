...
### NAME: ADITYA  
### DBMS Lab – Date Functions and CASE Statement  

### Aim  
To perform SQL queries using date functions, formatting functions, and CASE statements.

---

### Theory  

SQL provides functions to handle date and time data.

- DATEDIFF() → Difference between dates  
- TIMESTAMPDIFF() → Difference in months/years  
...
### 1. Display empno, ename and department name using CASE  
~~~sql
SELECT EMPNO, ENAME,
CASE DEPTNO
...
~~~sql
SELECT DATEDIFF(CURDATE(),'2005-09-04') AS AGE_IN_DAYS;
~~~
...
~~~sql
SELECT TIMESTAMPDIFF(MONTH,'2005-09-04',CURDATE()) AS AGE_IN_MONTHS;
~~~
...
~~~sql
SELECT DATE_SUB(CURDATE(),INTERVAL 3 MONTH) AS DATE_3_MONTH_BEFORE;
~~~
...
---
### 10. Employees where day of hire date equals last two digits of salary  
~~~sql
SELECT ENAME
FROM EMPLOYEE_MASTER
...
### 11. Employees whose 10% salary equals joining year  
~~~sql
SELECT ENAME
FROM EMPLOYEE_MASTER
...
### 12. Employees who joined before 15th of month  
~~~sql
SELECT *
FROM EMPLOYEE_MASTER
...
### 13. Employees who joined after 15th of month  
~~~sql
SELECT *
FROM EMPLOYEE_MASTER
WHERE DAY(HIREDATE) > 15;
...
### 14. Employees whose hire date is not NULL  
~~~sql
SELECT *
FROM EMPLOYEE_MASTER
